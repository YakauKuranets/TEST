  ...
  init: ({ elements, actions, state }) => {
    ...
    const createRuntimeAdapters = () => {
      ...
      return {
        ...
        local: {
          id: "local",
          modelVersion: "backend-gpu-1.0.0",
          detectFaces: async () => {
            try {
              ...
              const resp = await fetch(`http://127.0.0.1:${port}/ai/face-id/analyze`, { method: 'POST', body: fd });
              if (resp.ok) {
                const data = await resp.json();
                return { faces: data.faces || [], boxes: (data.faces || []).map(f => f.bbox), provider: "local" };
              }
            } catch (e) { console.warn('[AI] Backend face-detect failed, fallback to mock:', e); }
            return await mockAdapter.detectFaces();
          },
          detectObjects: async () => {
            try {
              const port = window.API_PORT || 8000;
              ...
              const fd = new FormData();
              fd.append('file', blob, 'frame.png');
              const resp = await fetch(`http://127.0.0.1:${port}/ai/segment`, { method: 'POST', body: fd });
              if (resp.ok) {
                const data = await resp.json();
                return { objects: data.objects || data.segments || [], provider: "local" };
              }
            } catch (e) { console.warn('[AI] Backend detect failed, fallback:', e); }
            return { objects: detectObjectsFromFrame() };
          },
          detectScenes: async () => runAutoSceneDelta(),
        },
      };
    ...
    const detectFaces = async () => {
      // Проверка доступности Face Detection API
      if (!('FaceDetector' in window) && state.aiProvider !== 'mock') {
        const message = '❌ Face Detection API не поддерживается в этом браузере.\n\n' +
                       'Используйте:\n' +
                       '• Google Chrome 91+\n' +
                       '• Microsoft Edge 91+\n\n' +
                       'Или переключитесь на режим "mock (demo)" в настройках AI runtime provider.';
        alert(message);
        setStatus("Face Detection API недоступен");
        return;
      }
      ...
      try {
        ...
        setStatus(`❌ Ошибка анализа лиц: ${errorMessage}`);
        console.error('[AI] Face detection failed:', error);
        alert(`Ошибка детекции лиц:\n\n${errorMessage}\n\nПроверьте консоль браузера (F12) для деталей.`);
